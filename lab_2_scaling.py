...
def scaling(list_of_homogeneous_coordinate, no_of_coord):
    ...
    matrix_for_scaling = [sx,0,0,0,sy,0,0,0,1]
    matrix_for_scaling = np.asarray(matrix_for_scaling).reshape(3,3)
    for i in range(len(list_of_homogeneous_coordinate)):
        new_scaled_coordinate.append(np.dot(matrix_for_scaling, list_of_homogeneous_coordinate[i]))
    ...
    scaled_homogeneous_coordinate = convert_to_hc(scaled_x_coordinate, scaled_y_coordinate, no_of_coord)
